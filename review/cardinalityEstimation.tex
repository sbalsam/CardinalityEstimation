...
claimed estimations and compare their results. This paper shows
...
fixed storage size. There are different approaches to solve this problem. Sampling
...
sources, but I will specify any equations, that are necessary to understand
...
Before the advent of the internet there were not as many data stream applications,
...
algorithm as described in Algorithm \ref{alg:fm85-add} to add an item by setting
the position of the rightmost '1' in a bitmap register.
...
higher number of registers available to build an average, the estimation quality raises,
...
is used, as shown in algorithm \ref{alg:fm85-query}. The correction factor 
...
for the algorithm for one register is $m \cdot 32$. That means with $m=256$, we can produce estimates
...
overestimate in the beginning. One solution for numbers below 1500 items is
...
a constant $\alpha$ is used to correct the bias against overestimations.
...
\cite{redis2025, dragonfly2025, redshift2025}. It is memory efficient and fast, as the
slowest part is the hashing function, it is scalable up to $10^9$ without loss of
...
In 2016, \cite{Quin2016} used the results of HLL++ to improve the algorithm
...
They claim they could reduce the error to $1.0/\sqrt{m}$, but \cite{Pettie2020} showed,
...
better than the plain HLL sketch. The plain HLL sketch needs about 1.5 more bits
...
and using the harmonic mean helps against an overestimation bias.
...
results of the included figures as I did not use cardinalities that would cause
problems due to hash collisions.
